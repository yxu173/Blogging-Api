...
public class DeleteUserHandler(UserServices userService)
    : IRequestHandler<DeleteUserCommand, OperationResult<bool>>
{
    ...
    )
    {
        ...
        catch (DeleteUserEx e)
        {
            e.ValidationErrors.ForEach(x =>
                _result.AddError(ErrorCode.UserDeletionFailed, e.Message)
            );
            return _result;
        }
